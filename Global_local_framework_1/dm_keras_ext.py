...
class DMMetrics(object):
    ...
    def sensitivity(y_true, y_pred):
        ...
        sen = tp / (pos + K.epsilon())
        print ('sensitivity', K.eval(sen))
    ...
    def specificity(y_true, y_pred):
        ...
        spe = tn / (neg + K.epsilon())
        print ('specificity', K.eval(spe))
    ...
    def accuracy(y_true, y_pred):
        ...
        pos = K.sum(K.round(K.clip(y_true, 0, 1)))
        neg = K.sum(K.round(K.clip(1-y_true, 0, 1)))
        acc = (tn+tp) / (pos+neg + K.epsilon())
        print('acc', K.eval(acc))
    ...
    def all_metrics(y_true, y_pred):
        ...
        spe = tn / (neg + K.epsilon())
        acc = (tn+tp) / (pos+neg + K.epsilon())
        print ('sensitivity, specificity, acc', K.eval(sen), K.eval(spe), K.eval(acc))

        return K.eval(sen), K.eval(spe), K.eval(acc)
...
class DMAucModelCheckpoint(Callback):
    ...
    def calc_test_auc(test_set, model, batch_size=None, test_samples=None,
        ...
        # for binary classification problems, only take one column to calculate TP, FN, TN, FP
        TP = (y_pred[:,0] * y_true[:,0]).sum()
        FN = ((1-y_pred[:,0]) * y_true[:,0]).sum()
        TN = ((1-y_pred[:,0]) * (1-y_true[:,0])).sum()
        FP = (y_pred[:,0] * (1-y_true[:,0])).sum()
        # ================================================================================= #
        ...
        if return_y_res:
            return (auc, y_true, y_pred, sensitivity, specificity, acc, TP, FN, TN, FP)

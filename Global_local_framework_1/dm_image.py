...
def read_resize_img(fname, target_size=None, target_height=None, 
                    target_scale=None, gs_255=False, rescale_factor=None, seg_breast=False, image_normalize=False, 
                    imageset_normalize=False, imageset_mean=0.0, imageset_std=0.0):
    ...
    if image_normalize:
        # image-level normalization: standardize image to zero mean unit std
        img -= img.mean()
        img /= (img.std()+np.finfo(float).eps)
    if imageset_normalize:
        #imageset-level normalization
        img -= imageset_mean
        img /= imageset_std
    
    # print ('after normalization: mean:{} +- std:{}'.format(img.mean(), img.std()))
    return img


def read_img_for_pred(fname, equalize_hist=False, data_format='channels_last', 
                      dup_3_channels=True,
                      transformer=None, standardizer=None, seg_breast=False, image_normalize=False,
                      imageset_normalize=False, imageset_mean=0.0, imageset_std=0.0, **kwargs):
    '''Read an image and prepare it for prediction through a network
    '''
    img = read_resize_img(fname, seg_breast=seg_breast, image_normalize=image_normalize, imageset_normalize=imageset_normalize, 
                          imageset_mean=imageset_mean, imageset_std=imageset_std, **kwargs)
    if equalize_hist:
    ...
    if not imageset_normalize and not image_normalize:
...
class DMDirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,
                 ...
                 save_to_dir=None, save_prefix='', save_format='jpeg',
                 follow_links=False, seg_breast=False, image_normalize=False, 
                 imageset_normalize=True, imageset_mean=0.0, imageset_std=0.0):
        '''
        Args:
        ...
        self.data_format = data_format
        self.seg_breast = seg_breast
        self.image_normalize = image_normalize
        self.imageset_normalize = imageset_normalize
        self.imageset_mean = imageset_mean
        self.imageset_std = imageset_std
    ...
    def next(self):
        ...
        for i, j in enumerate(index_array):
            ...
            x = read_img_for_pred(
                ...
                gs_255=self.gs_255, rescale_factor=self.rescale_factor, seg_breast=self.seg_breast,
                image_normalize=self.image_normalize, imageset_normalize=self.imageset_normalize, 
                imageset_mean=self.imageset_mean, imageset_std=self.imageset_std)
            batch_x[i] = x
        # optionally save augmented images to disk for debugging purposes
        if self.save_to_dir:
            for i in range(current_batch_size):
...
class DMImageDataGenerator(ImageDataGenerator):
    ...
    def flow_from_directory(self, directory,
        ...
        return DMDirectoryIterator(
            ...
            seg_breast=seg_breast,
            image_normalize=image_normalize,
            imageset_normalize=imageset_normalize,
            imageset_mean=imageset_mean,
            imageset_std=imageset_std)

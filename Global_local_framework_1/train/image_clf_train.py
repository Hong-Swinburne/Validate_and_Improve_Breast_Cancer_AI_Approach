...
def run(train_dir, val_dir, test_dir, patch_model_state=None, resume_from=None,
        ...
        seg_breast = False,
        image_normalize=False, imageset_normalize=False,
        imageset_mean=32.41, imageset_std=40.64,
    ...
    if load_train_ram:
        ...
        raw_generator = raw_imgen.flow_from_directory(
            ...
            batch_size=train_bs, shuffle=False, seg_breast = False, image_normalize=image_normalize, 
            imageset_normalize=imageset_normalize, imageset_mean=imageset_mean, imageset_std=imageset_std)
        ...
        train_generator = train_imgen.flow(
            ...
            shuffle=True, seed=random_seed, seg_breast = False, image_normalize=image_normalize, 
            imageset_normalize=imageset_normalize, imageset_mean=imageset_mean, imageset_std=imageset_std)
    else:
        print "Create generator for train set"
    ...
    validation_set = val_imgen.flow_from_directory(
        ...
        equalize_hist=equalize_hist, dup_3_channels=dup_3_channels,
        classes=class_list, class_mode='categorical', 
        batch_size=batch_size, shuffle=False, seg_breast = False, image_normalize=image_normalize, 
        imageset_normalize=imageset_normalize, imageset_mean=imageset_mean, imageset_std=imageset_std)
    sys.stdout.flush()
    if load_val_ram:
        print "Loading validation set into RAM.",
        sys.stdout.flush()
        validation_set = load_dat_ram(validation_set, validation_set.nb_sample, seg_breast = False, 
                                    image_normalize=image_normalize, imageset_normalize=imageset_normalize, 
                                    imageset_mean=imageset_mean, imageset_std=imageset_std)
        print "Done."; sys.stdout.flush()
    ...
    test_generator = test_imgen.flow_from_directory(
        ...
        shuffle=False, seg_breast = False, image_normalize=image_normalize, 
        imageset_normalize=imageset_normalize, imageset_mean=imageset_mean, imageset_std=imageset_std)
    ...
    test_auc, _, _, sensitivity, specificity, acc, _, _, _, _ = DMAucModelCheckpoint.calc_test_auc(
        test_generator, image_model, test_samples=test_samples, return_y_res=True)
    print "Test_AUROC, sen, spe, acc, on test set:", test_auc, sensitivity, specificity, acc
    ...
    train_generator = train_imgen.flow_from_directory(
            ...
            equalize_hist=equalize_hist, dup_3_channels=dup_3_channels,
            classes=class_list, class_mode='categorical', batch_size=batch_size, 
            shuffle=True, seg_breast = False, image_normalize=image_normalize, 
            imageset_normalize=imageset_normalize, imageset_mean=imageset_mean, imageset_std=imageset_std)
    ...
    train_res = image_model.evaluate_generator(
        train_generator, train_steps, nb_worker=nb_worker,
        pickle_safe=True if nb_worker > 1 else False)

    train_auc, _, _, sensitivity, specificity, acc, _, _, _, _ = DMAucModelCheckpoint.calc_test_auc(
        train_generator, image_model, test_samples=train_samples, return_y_res=True)
    print "Train_AUROC, sen, spe, acc, on train set:", train_auc, sensitivity, specificity, acc
    #=====================================================================================================
...
if __name__ == '__main__':
    ...
    parser.add_argument("--featurewise-mean", dest="featurewise_mean", type=float, default=32.41)
    parser.add_argument("--image_normalize", dest="image_normalize", type=bool, default=False)
    parser.add_argument("--imageset_normalize", dest="imageset_normalize", type=bool, default=False)
    parser.add_argument("--imageset_mean", dest="imageset_mean", type=float, default=32.41)
    parser.add_argument("--imageset_std", dest="imageset_std", type=float, default=40.64)
    parser.add_argument("--seg_breast", dest="seg_breast", type=bool, default=False)
    ...
    parser.add_argument("--batch-size", "-bs", dest="batch_size", type=int, default=8)
    ...
    run_opts = dict(
        ...
        imageset_normalize=args.imageset_normalize,
        imageset_mean=args.imageset_mean,
        imageset_std=args.imageset_std,
        seg_breast=args.seg_breast,
